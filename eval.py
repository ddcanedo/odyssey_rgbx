...
from utils.visualize import print_iou, show_img, get_colors
...
class SegEvaluator(Evaluator):
    def func_per_iteration(self, data, device):
        ...
        if self.save_path is not None:
            ...
            '''class_colors = get_colors(config.num_classes)
            palette_list = list(np.array(class_colors).flat)
            if len(palette_list) < 768:
                palette_list += [0] * (768 - len(palette_list))
            palette_list = [int(num) for num in palette_list]
            result_img.putpalette(palette_list)
            result_img.save(os.path.join(self.save_path+'_color', fn))'''

            # save raw result
            # Binarize the mask because of the anti-aliasing problem
            result_img = np.array(result_img)
            result_img[result_img > 0] = 255
            cv2.imwrite(os.path.join(self.save_path, fn), result_img)
            logger.info('Save the image ' + fn)

        if self.show_image:
            colors = self.dataset.get_colors
            image = img
