...
class Evaluator(object):
    ...
    def run(self, model_path, model_indice, log_file, log_file_link):
        ...
        for model in models:
            ...
            if len(self.devices ) == 1:
                result_line, _ = self.single_process_evalutation()
            else:
    ...
    def run_for_each_epoch(self, model):
        self.val_func = model
        result_line, (iou, mean_IoU, freq_IoU, mean_pixel_acc, pixel_acc) = self.single_process_evalutation()
        return (iou, mean_IoU, freq_IoU, mean_pixel_acc, pixel_acc)


    '''def load(self, model):
        self.val_func = load_model(self.network, model)

    def predict(self, img, modal_x, label):
        results_dict, result_image = self.predict_one_by_one(img, modal_x, label, self.devices[0])
        return results_dict, result_image

    def compute_metrics(self, all_results):
        result_line, (iou, mean_IoU, freq_IoU, mean_pixel_acc, pixel_acc) = self.compute_metric(all_results)
    ...
    def multi_process_evaluation(self):
        ...
        result_line, _ = self.compute_metric(all_results)
        logger.info(
            'Evaluation Elapsed Time: %.2fs' % (
                    time.perf_counter() - start_eval_time))
        return result_line
